...
点名 `idea-to-project-structure` 后，不加模式词，默认进入第一种能力：新项目结构生成。它会先问少量会影响结构的问题，然后给出推荐形态、技术栈、模块边界和目录结构。
...
如果你不只要目录结构，还想把核心文档一起生成出来，说 `full-docs`、`完整文档模式`、`完整文档`、`核心文档一起生成` 或“结构和文档一起落盘”，进入第二种能力。

进入完整文档模式后，不再反问“只输出文档还是创建骨架和文档”。这个模式默认就是要生成结构和核心文档；只会继续确认影响文档内容的关键信息，以及需要落盘时的项目名和目标路径。

确认执行方式、技术栈和落盘位置之后，仍然要先问一轮需求问题，不能直接脑补完整需求然后创建文件。至少要确认 MVP、优先识别的对象/文件类型、v1 不做什么。
...
如果是已有仓库，不想重构代码，只想补一层 AI 友好的启动链和文档结构，说 `重构仓库`、`重构这个仓库`、`retrofit`、`retrofit-existing-project`、`旧项目改造`、`现有仓库`、`当前仓库`，或直接给出已有项目路径，进入第三种能力。
